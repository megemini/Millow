...
- **颜色** — 灰度（平坦与加权）、反色、色调、BGR、Alpha 展平、HSV/YCbCr 转换、
  LUT 应用、`over`/`color` 合成。
- **几何变换** — 裁剪、翻转、90/180/270 旋转、任意角度旋转、平移、仿射变换、
  错切、透视变换（8 自由度单应矩阵 + 4 点便捷接口）、缩放（最近邻/双线性/双三次）、
  等比缩放、适配/覆盖、`resize_to_model_input`、`letterbox`、缩略图、填充。
- **增强** — 亮度、对比度、伽马、归一化、标准化、自动对比度、锐化、Unsharp Mask。
- **阈值与直方图** — 固定阈值、Otsu、Sauvola、Niblack、Bernsen、Wolf、
  自适应阈值（均值/高斯）、直方图（灰度与彩色）、均衡化、CLAHE、直方图匹配。
- **滤波与边缘** — 卷积、均值/高斯模糊、中值/最小/最大值滤波、双边滤波、
  **引导滤波**（He 等 2010，基于积分图 O(n)）、**Wiener 滤波**（自适应 MMSE 去噪）、
  **高斯差分（DoG）**、Sobel、Scharr、Prewitt、Laplacian、Canny。
- **形态学** — 腐蚀、膨胀、开运算、闭运算、形态学梯度、Top-hat、Black-hat、
  骨架化（Zhang-Suen）、击中击不中、`remove_small_objects`/`remove_small_holes`、
  `remove_lines`（方向性表格线去除）。
- **特征检测** — LBP、HOG、Harris 角点、Shi-Tomasi 角点。
- **测量** — 连通域、轮廓提取、矩、Hu 矩、区域属性、像素计数、
  **Hough 直线变换**、**距离变换**、**模板匹配**（完整结果图 + 最佳匹配位置）、
  **K-means 颜色量化**。
